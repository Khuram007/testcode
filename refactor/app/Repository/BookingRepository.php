...
class BookingRepository extends BaseRepository
{
    ...
    /**
     * @param  Job  $model
     * @param  \DTApi\Mailers\MailerInterface  $mailer
     *
    ...
    function __construct(Job $model, MailerInterface $mailer)
    {
        ...
        $this->logger->pushHandler(new StreamHandler(storage_path('logs/admin/laravel-'.date('Y-m-d').'.log'),
            Logger::DEBUG));
    ...
    public function getUsersJobs($user_id): array
    {
        ...
        $emergencyJobs = $noramlJobs = [];
        if ($cuser->is('customer')) {
            $jobs = $cuser->jobs()
                          ->with('user.userMeta', 'user.average', 'translatorJobRel.user.average', 'language',
                              'feedback')
    ...
    public function getUsersJobsHistory($user_id, Request $request): array
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $data['emergencyJobs'] = [];
            $data['noramlJobs']    = $jobs_ids;
            $data['jobs']          = $jobs_ids;
            $data['cuser']         = $cuser;
            $data['usertype']      = $usertype;
            $data['numpages']      = ceil($jobs_ids->total() / 15);
     ...
     * @return mixed
     */
    public function store($user, BookingStore $request)
    {
        $response      = [];
        $immediatetime = 5;
    ...
    public function getPotentialJobIdsWithUserId($user_id): array
    {
        ...
        if ($translator_type == 'professional') {
            $job_type = 'paid';
        }   /*show all jobs for professionals.*/
        elseif ($translator_type == 'rwstranslator') {
            $job_type = 'rws';
        }  /* for rwstranslator only show rws jobs. */
        ...
        $jobIDs           = Job::getJobs($user_id, $job_type, 'pending', $userlanguage, $gender, $translator_level);
    ...
    public function sendNotificationTranslator($job, $data = [], $exclude_user_id)
    {
        ...
        $logger->addInfo('Push send for job '.$job->id,
            [$translator_array, $delpay_translator_array, $msg_text, $data]);
        $this->sendPushNotificationToSpecificUsers($translator_array, $job->id, $data, $msg_text,
            false);       // send new booking push to suitable translators(not delay)
        $this->sendPushNotificationToSpecificUsers($delpay_translator_array, $job->id, $data, $msg_text,
            true); // send new booking push to suitable translators(need to delay)
    }
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $this->logger->addInfo('USER #'.$cuser->id.'('.$cuser->name.')'.' has been updated booking <a class="openjob" href="/admin/jobs/'.$id.'">#'.$id.'</a> with data:  ',
            $log_data);
        ...
        if ($job->due <= Carbon::now()) {
            ...
            if ($changeTranslator['translatorChanged']) {
                $this->sendChangedTranslatorNotification($job, $current_translator,
...
//        }
    ...
    private function changePendingStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data['status'] == 'assigned' && $changedTranslator) {
            ...
            $this->mailer->send($translator->email, $translator->name, $subject,
                'emails.job-changed-translator-new-translator', $dataEmail);
            ...
            $this->mailer->send($email, $name, $subject, 'emails.status-changed-from-pending-or-assigned-customer',
                $dataEmail);
            $job->save();
    ...
    {
        $this->logger->pushHandler(new StreamHandler(storage_path('logs/cron/laravel-'.date('Y-m-d').'.log'),
            Logger::DEBUG));
        $this->logger->pushHandler(new FirePHPHandler());
        $data                      = array();
        ...
        if ($this->bookingRepository->isNeedToSendPush($user->id)) {
            ...
            $this->bookingRepository->sendPushNotificationToSpecificUsers($users_array, $job->id, $data, $msg_text,
    ...
    private function changeTranslator($current_translator, $data, $job)
    {
        ...
        if ( ! is_null($current_translator) || (isset($data['translator']) && $data['translator'] != 0) || $data['translator_email'] != '') {
            ...
            if ($translatorChanged) {
                return [
                    'translatorChanged' => $translatorChanged, 'new_translator' => $new_translator,
                    'log_data'          => $log_data,
                ];
            }
    ...
    public function sendExpiredNotification($job, $user)
    {
        ...
        if ($this->isNeedToSendPush($user->id)) {
            $users_array = array($user);
            $this->sendPushNotificationToSpecificUsers($users_array, $job->id, $data, $msg_text,
    ...
    public function acceptJobWithId($job_id, $cuser)
    {
        ...
        if ( ! Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                if ($this->isNeedToSendPush($user->id)) {
                    $users_array = array($user);
                    $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text,
                        $this->isNeedToDelayPush($user->id));
                }
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                ...
                if ($this->isNeedToSendPush($translator->id)) {
                    ...
                    $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text,
                        $this->isNeedToDelayPush($translator->id));// send Session Cancel Push to Translaotor
                }
            }
        } else {
            if ($job->due->diffInHours(Carbon::now()) > 24) {
                ...
                if ($customer) {
                    ...
                    if ($this->isNeedToSendPush($customer->id)) {
                        ...
                        $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $data, $msg_text,
                            $this->isNeedToDelayPush($customer->id));     // send Session Cancel Push to customer
                    }
                }
                $job->status         = 'pending';
                $job->created_at     = date('Y-m-d H:i:s');
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        $jobIDs = Job::getJobs($cuser->id, $job_type, 'pending', $userlanguage, $gender, $translator_level);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            ...
            if (isset($requestdata['translator_email']) && count($requestdata['translator_email'])) {
                ...
                if ($users) {
                    $allJobIDs = DB::table('translator_job_rel')->whereNull('cancel_at')->whereIn('user_id',
                        collect($users)->pluck('id')->all())->lists('job_id');
    ...
    public function alerts()
    {
        ...
        return [
            'allJobs'         => $allJobs, 'languages' => $languages, 'all_customers' => $all_customers,
            'all_translators' => $all_translators, 'requestdata' => $requestdata,
        ];
    }
